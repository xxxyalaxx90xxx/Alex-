...
check_prerequisites() {
  ...
  if command_exists df; then
    local var_space_kb
    var_space_kb=$(df /var 2>/dev/null | awk 'NR==2 {print $4}')
...
run_health_checks() {
  ...
  if command_exists kubectl; then
    ...
    if node_output=$(kubectl --kubeconfig="${KUBECONFIG_FILE}" get nodes -o wide 2>&1); then
      echo "$node_output"
    ...
    if component_output=$(kubectl --kubeconfig="${KUBECONFIG_FILE}" get componentstatuses 2>&1); then
